...
class PathTest < Test::Unit::TestCase
  ...
  def test_path_without_type
    result = FluentPath::STU3.evaluate('name.given', PATIENT)
    assert result == NAMES + WTF, 'Failed to navigate path.'
  end

  def test_path_with_type
    result = FluentPath::STU3.evaluate('Patient.name.given', PATIENT)
    assert result == NAMES + WTF, 'Failed to navigate path.'
  end

  def test_path_conversion_2args
    result = FluentPath::STU3.evaluate('Patient.name.given.select(substring(0,3))', PATIENT)
    assert result == %w(Bob Rob Bob Rob), 'Failed to navigate path.'
  end

  def test_path_conversion_1args
    result = FluentPath::STU3.evaluate('Patient.name.given.select(substring(1))', PATIENT)
    assert result == %w(ob obert obert ob), 'Failed to navigate path.'
  end

  def test_path_with_quotes
    result = FluentPath::STU3.evaluate('Message."PID-1"', MESSAGE)
    assert result == PID1, 'Failed to navigate path.'
  end

  def test_array_access
    result = FluentPath::STU3.evaluate('Array[0]', ARRAY)
    assert result == 'A', 'Failed to navigate path.'
  end

  def test_array_access_with_variable
    result = FluentPath::STU3.evaluate('Array[index]', ARRAY)
  ...
  def test_children_first
    ...
    result = FluentPath::STU3.evaluate(expression, data)
  ...
  def test_parent
    ...
    data = {}
    parent = { 'type' => 'integer' }
    result = FluentPath::STU3.evaluate(expression, data, parent)

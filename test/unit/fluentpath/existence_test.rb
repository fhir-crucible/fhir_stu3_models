...
class ExistenceTest < Test::Unit::TestCase
  ...
  def test_empty_false
    result = FluentPath::STU3.evaluate('Patient.name.given.empty()', PATIENT)
    assert result == false, 'Failed empty test.'
  end

  def test_empty_true
    result = FluentPath::STU3.evaluate('Patient.name.family.empty()', PATIENT)
    assert result == true, 'Failed empty test.'
  end

  # ------------------------- not() -------------------------------------
  def test_not_true
    result = FluentPath::STU3.evaluate('Patient.deceasedBoolean.not()', PATIENT)
    assert result == true, 'Failed not test.'
  end

  def test_not_false
    result = FluentPath::STU3.evaluate('Patient.aliveBoolean.not()', PATIENT)
    assert result == false, 'Failed not test.'
  end

  def test_not_empty
    result = FluentPath::STU3.evaluate('Patient.doesNotExist.not()', PATIENT)
    assert result == true, 'Failed not test.'
  end

  def test_not_notempty
    result = FluentPath::STU3.evaluate('Patient.name.given.not()', PATIENT)
    assert result == false, 'Failed not test.'
  end

  # ------------------------- exists() -------------------------------------
  def test_exists_true
    result = FluentPath::STU3.evaluate('Patient.name.given.exists()', PATIENT)
    assert result == true, 'Failed exists test.'
  end

  def test_exists_false
    result = FluentPath::STU3.evaluate('Patient.name.family.exists()', PATIENT)
    assert result == false, 'Failed exists test.'
  end

  # ------------------------- all() -------------------------------------
  def test_all_true
    result = FluentPath::STU3.evaluate('Patient.all_true.all()', PATIENT)
    assert result == true, 'Failed all test.'
  end

  def test_all_false
    result = FluentPath::STU3.evaluate('Patient.some_true.all()', PATIENT)
  ...
  def test_distinct_strings
    result = FluentPath::STU3.evaluate('Patient.codes.distinct()', PATIENT)
    assert result == %w(A B), 'Failed distinct test.'
  end

  def test_distinct_numbers
    result = FluentPath::STU3.evaluate('Patient.numbers.distinct()', PATIENT)
    assert result == [0.1, 1, -2], 'Failed distinct test.'
  end

  def test_distinct_booleans
    result = FluentPath::STU3.evaluate('Patient.some_true.distinct()', PATIENT)
  ...
  def test_in_numbers
    ...
    result = FluentPath::STU3.evaluate('foo in set', data)
  ...
  def test_set_creation
    data = {}
    result = FluentPath::STU3.evaluate("('A'|'B'|'C')", data)

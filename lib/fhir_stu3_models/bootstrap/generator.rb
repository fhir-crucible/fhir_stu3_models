    ...
    module Boot
      class Generator
        ...
        def generate_metadata
          template = FHIR::STU3::Boot::Template.new([], true)

          primitives = @defn.primitive_types
          hash = {}
          primitives.each do |p|
            field = FHIR::STU3::Field.new
            field.name = nil
        ...
        def generate_class(hierarchy, structure_def, top_level = false)
          ...
          template = FHIR::STU3::Boot::Template.new([type_name], top_level)
          ...
          structure_def['snapshot']['element'].each do |element|
            ...
            if !element['type'].nil?
              ...
              element['type'].map { |t| t['code'] }.uniq.each do |data_type|
                ...
                field = FHIR::STU3::Field.new(fieldname)

  ...
  describe '#is_primitive?(datatype, value)' do
    ...
    describe "datatype 'integer'" do
      ...
      context 'returns true when' do
        let(:result) { true }
        specify 'value= 0' do
          expect(subject.is_primitive?(datatype, 0)).to be result
        end
        specify 'value= -1' do
          expect(subject.is_primitive?(datatype, -1)).to be result
        end
        specify 'value= 1' do
          expect(subject.is_primitive?(datatype, 1)).to be result
        end
      ...
      context 'returns false when' do
        ...
        specify "value= '1.0'" do
          expect(subject.is_primitive?(datatype, '1.0')).to be result
        end
        specify "value= '+1'" do
        ...
        specify "value= 'non-numeric-string'" do
          expect(subject.is_primitive?(datatype, 'non-numeric-string')).to be result
        end
        specify "value= 'string with trailing 123'" do
          expect(subject.is_primitive?(datatype, 'string with trailing 123')).to be result
        end
        specify "value= '123 string with leading number'" do
          expect(subject.is_primitive?(datatype, '123 string with leading number')).to be result
        end
        specify 'value= nil' do
          expect(subject.is_primitive?(datatype, nil)).to be result
        end
        specify "value= '1.5'" do
          expect(subject.is_primitive?(datatype, '1.5')).to be result
        end
        specify 'value= 1.5' do
          expect(subject.is_primitive?(datatype, 1.5)).to be result
        end
    ...
    describe "datatype: 'decimal'" do
      ...
      context 'returns false when' do
        ...
        specify "value= '001'" do
          ...
          expect(subject.is_primitive?(datatype, '001')).to be result
        end
        specify "value= '1E3'" do
          pending('FHIR standard forbids exponents (<=1.8.0)')
          expect(subject.is_primitive?(datatype, '1E3')).to be result
        end
      end
    end
  end
